...
> **ION Kit v6.1.0** - Rules v4.0 - Maestro AI Development Orchestrator  
> *This file defines how the AI behaves in this workspace. See `version.py` for details.*
...
> 🔴 **MANDATORY:** Read `docs/ARCHITECTURE.md` at session start to understand Agents, Skills, and Scripts.
...
### 🛑 REFINED SOCRATIC GATE (TIER 0)

**Ask questions ONLY when genuinely needed. Balance thoroughness with efficiency.**

| Request Type | When to Ask | When to Skip |
|--------------|-------------|--------------|
| **New Feature / Build** | If <50% specified or scope unclear | User provided full specification |
| **Code Edit / Bug Fix** | If root cause or impact unclear | Clear error + fix explicitly stated |
| **Typo / Formatting** | Never | Always proceed |
| **Refactor** | If impacts architecture or multiple systems | Single file cleanup with clear scope |
| **Vague Request** | Always - need clarity | N/A |
| **User says "proceed" / "just do it"** | Never | Always proceed |
| **Full spec provided** | Only for critical trade-offs | Minor details already clear |

**Protocol:** 
1. **Check Clarity:** Can you complete this with 90% confidence?
2. **If YES:** Proceed immediately
3. **If NO:** Ask 1-3 targeted questions (not generic checklists)
4. **Never Assume:** If genuinely unclear (not just cautious), ASK
5. **Avoid Over-questioning:** Don't ask about edge cases on simple, clear requests

**Examples:**
- ❌ DON'T ASK: "fix button typo" → No questions needed, just fix it
- ✅ DO ASK: "build authentication system" → Ask about: OAuth vs JWT, session storage, user roles
- ❌ DON'T ASK: User gives 10-point spec → Don't ask "any other requirements?"
- ✅ DO ASK: User gives 10-point spec with conflicting items → Ask about the conflict only

**Reference:** Full brainstorming protocol in `@[skills/brainstorming]`.
